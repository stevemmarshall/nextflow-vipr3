...
if(params.help) {
    log.info ''
    log.info '¯\\_(ツ)_/¯ ViPR3 Pipeline (SPades) ¯\\_(ツ)_/¯'
    log.info ''
    log.info ''
...
if (params.qc){
    ...
    output:
    set read_id, file("${read_id}_R1.trimmed.fastq.gz"), file("${read_id}_R2.trimmed.fastq.gz") into qc_bbnorm, ch_map
    file("${read_id}_fastp.html")
    ...
    read_pairs.into{qc_bbnorm; ch_map}
    }
...
process bbnorm {
    ...
    input:
    set read_id, file(forward), file(reverse) from qc_bbnorm

    output:
    set read_id, file("${read_id}_R1.norm.fastq.gz"), file("${read_id}_R2.norm.fastq.gz") into norm_spades
    """
    bbnorm.sh -Xmx8g\
    in=$forward \
    in2=$reverse \
    out=${read_id}_R1.norm.fastq.gz \
    out2=${read_id}_R2.norm.fastq.gz
    """
}

process spades {

    publishDir "$params.output/$read_id/spades", mode: "copy"

    tag { read_id }

    input:
    set read_id, file(forward), file(reverse) from norm_spades
    output:
    set read_id, file("${read_id}_spades_contigs.fasta"), file("${read_id}_spades_scaffolds.fasta") into (ch_spades, ch_spades_nucmer_qc)
    file("spades.log")
    ...
    -o asm \
    >& spades.log
    mv asm/contigs.fasta ${read_id}_spades_contigs.fasta
    mv asm/scaffolds.fasta ${read_id}_spades_scaffolds.fasta
    """
}
...
process join_contigs {
    ...
    input:
    set read_id, file(contig), file(scaffolds) from ch_spades
    file(ref)
    ...
    -s ${read_id}_gap_filled_spade >& join_contig.log
    """
}
...
process lofreq_call {
    ...
    input:
    // set read_id, file(contigs) from joined_contigs_lofreq
...
process nucmer_qc {
    ...
    input:
    set read_id, file(comtigs), file(scaffolds) from ch_spades_nucmer_qc
    file(ref)
